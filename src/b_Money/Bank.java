...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			//accountlist.get(accountid); // it doesn't add account, but tries to get it
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) { //added ! sign, so if account does not exist, then we throw an Exception
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			//account.deposit(money); it doesn't withdraw money, but tries to deposit them
			account.withdraw(money); 
		}
		/*in case of withdraw, we can add if statement to check whether amount to be withdrawn 
		 * bigger than balance and cancel operation if it is so, but since we had negative Money value
		 * I assume, that we allow to go bellow 0*/
	}
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		...
		// third parameter should be toaccount, not fromaccount
		 //  if statement can be added, to check whether fromaccount and toaccount exist 
	...
	public void addTimedPayment(String accountid, String payid, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) {
		//if(accountlist.contains(accountid)) {
			Account account = accountlist.get(accountid);
			if(!account.timedPaymentExists(payid)) account.addTimedPayment(payid, interval, next, amount, tobank, toaccount);
		//}
		// 2 if statements can be added, to check whether account we looking for exists, 
		// and whether timed payment we want to add doesn't exists yet
	}
	...
	public void removeTimedPayment(String accountid, String id) {
		if(accountlist.contains(accountid)) {
			Account account = accountlist.get(accountid);
			if(account.timedPaymentExists(id)) account.removeTimedPayment(id);
		}
		// 2 if statements can be added, to check whether account we looking for exists, 
		// and whether timed payment we want to remove exists
	}
